...
public class ArchiveActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> archiveNotesListMap = new ArrayList<>();
	
	private LinearLayout toolbarBg;
	private NestedScrollView mainScroll;
	private ImageView toolbarBackImage;
	private LinearLayout toolbarTextBg;
	private ImageView toolbarUnarchiveImage;
	private TextView toolbarTitle;
	private TextView toolbarSubtitle;
	private LinearLayout mainBg;
	private LinearLayout archiveTitleBg;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		toolbarUnarchiveImage = findViewById(R.id.toolbarUnarchiveImage);
		toolbarTitle = findViewById(R.id.toolbarTitle);
		toolbarSubtitle = findViewById(R.id.toolbarSubtitle);
		mainBg = findViewById(R.id.mainBg);
		archiveTitleBg = findViewById(R.id.archiveTitleBg);
		archiveInfoText = findViewById(R.id.archiveInfoText);
		divider = findViewById(R.id.divider);
		mainRecycler = findViewById(R.id.mainRecycler);
		emptyLottie = findViewById(R.id.emptyLottie);
		archiveTitleText = findViewById(R.id.archiveTitleText);
		...
		toolbarUnarchiveImage.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				final AlertDialog dialog = new AlertDialog.Builder(ArchiveActivity.this).create();
				...
				infoAlertDialogText.setText(getString(R.string.restoreAllArchivedNotes));
				...
				if (archiveNotesListMap.size() == 0) {
					okAlertDialogButton.setEnabled(false);
				}
				okAlertDialogButton.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){
						...
						final Runnable rwait  = new Runnable() {
							...
							public void run() {
								if (archiveNotesListMap.size() > 0) {
									while(archiveNotesListMap.size() > 0) {
										noteValues = archiveNotesListMap.get((int)0);
										noteValues.put("state", "Normal");
										Database_update("notes", noteValues, "id" + " = ?", new String[]{noteValues.get("id").toString()});
										archiveNotesListMap.remove((int)(0));
									}
									SketchwareUtil.showMessage(getApplicationContext(), getString(R.string.allArchivedNotesRestored));
	...
	private void initializeLogic() {
		Database = SQLiteDatabase.openOrCreateDatabase((ArchiveActivity.this).getDatabasePath("QuickNotes" + ".db").getPath(), null);
	...
	public void _GetDataFromSQLite() {
		try{
			archiveNotesListMap = Database_retrieveMultiple("notes", "id, title, note, tags, isFavorite, category, state, sortTime, createTime, editTime, trashTime, archiveTime, isReminder, reminderTime".split(", "), "state" + " = ?", new String[]{"Archive"});
			SketchwareUtil.sortListMap(archiveNotesListMap, "archiveTime", false, false);
			mainRecycler.setAdapter(new MainRecyclerAdapter(archiveNotesListMap));
			if (archiveNotesListMap.size() == 0) {
